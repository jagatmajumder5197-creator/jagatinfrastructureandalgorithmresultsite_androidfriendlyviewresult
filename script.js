...
function renderResult(student) {
  ...
  // Print Display
  document.getElementById('printStudentName').innerText = student.STUDENTS_NAME || '';
  document.getElementById('printFatherName').innerText = student.FATHERS_NAME || '';
  document.getElementById('printStudentClass').innerText = student.CLASS || '';
  document.getElementById('printRollNumber').innerText = student.ROLL || '';
  ...
  subjects.forEach(sub => {
    ...
    mobileRow.className = 'subject-row';
    mobileRow.innerHTML = `
      <div class="subject-name">${sub.name}</div>
      <div class="marks-grid">
        <div class="mark-item">
          <span class="label">Written:</span>
          <span class="value">${obtainedWritten}/${fmStructure.written}</span>
        </div>
        <div class="mark-item">
          <span class="label">Oral:</span>
          <span class="value">${obtainedOral}/${fmStructure.oral}</span>
        </div>
        <div class="mark-item">
          <span class="label">Total:</span>
          <span class="value">${total}/${fm}</span>
        </div>
        <div class="mark-item">
    ...
    `;
    subjectsList.appendChild(mobileRow);
  ...
  document.getElementById('grandResult').innerText = hasFailed ? 'FAIL' : 'PASS';
  document.getElementById('grandRank').innerHTML = formatOrdinal(calculatedRank);

  // Print Display
  const artVal = student.ART || student.Art || 'A<sup>+</sup>';
  document.getElementById('printArtGrade').innerHTML = artVal;
  document.getElementById('printGrandFullMarks').innerText = grandFullMarks;
  document.getElementById('printGrandTotal').innerText = grandObtained;
  document.getElementById('printGrandPercentage').innerText = grandPercentage.toFixed(2) + '%';
  document.getElementById('printGrandGrade').innerText = grandGrade;
  document.getElementById('printGrandResult').innerText = hasFailed ? 'Fail' : 'Pass';
  document.getElementById('printGrandRank').innerHTML = formatOrdinal(calculatedRank);
}
...
function downloadPDF() {
  ...
  const studentName = currentStudent.STUDENTS_NAME || 'Student';
